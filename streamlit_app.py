...
with st.sidebar:
    ...
    country = st.selectbox("Select Country", ["Morocco", "Algeria", "Tunisia", "Libya", "Egypt", "Syria", "Lebanon", "Yemen", "Mauritania"], index=6)
...
if st.button("Run Analysis"):
    try:
        ...
        for img_key, fname in zip(
            ...
            st.session_state["exported_files"][img_key] = fname
...
if st.session_state.get("exported_files"):
    ...
    for img_key, label in zip(
        ["GES_diff", "GES_first", "GES_last"],
        ["Download GES Change", "Download GES Start Year", "Download GES End Year"]
    ):
        filepath = st.session_state["exported_files"].get(img_key)
